...
\item \code{sub\_vector\_space }: Definition of a vector subsace and the vector space structure on it.
\item
\end{itemize}
The respective notions are defined in the canonical way and the proofs are of course trivial from the algebra standpoint. Thus, there is no need for huge lambda terms (which are otherwise are not uncommon in lean texts) and the code is fairly easy to understand.
