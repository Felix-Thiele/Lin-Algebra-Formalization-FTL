...
After the definition of various structures on sets we define linear functions. We introduce the set of homomorphisms immediately as the carrier set of a new structure Hom(K,V,W).
\begin{lstlisting}
...
\end{lstlisting}
The {\ftl structure} approach allows us to also regard Hom(K,V,V) as a ring, which is done in the endomorphism files.
...
Axiom. smul{K} = mul{K}.
\end{lstlisting}
We then prove in the the P\_field2VS file that this really does create a vector space:
\begin{lstlisting}
